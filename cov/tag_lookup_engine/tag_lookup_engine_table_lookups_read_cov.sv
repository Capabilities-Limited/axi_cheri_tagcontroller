...
module tag_lookup_engine_table_lookups_read_cov #(
...
, input logic [SB_IDX_W-1:0] retire_ptr_q
  ...
  // Interface Handshakes
  //============================================================================
  `DEF_VALID_READY_COV(read_incoming_req, clk_i, rst_ni, req_valid_i, req_ready_o)
  `DEF_VALID_READY_COV(read_outgoing_resp, clk_i, rst_ni, resp_valid_o, resp_ready_i)
  `DEF_VALID_READY_COV(read_root_req, clk_i, rst_ni, root_req_valid_o, root_req_ready_i)
  `DEF_VALID_READY_COV(read_root_resp, clk_i, rst_ni, root_resp_valid_i, root_resp_ready_o)
  `DEF_VALID_READY_COV(read_leaf_req, clk_i, rst_ni, leaf_req_valid_o, leaf_req_ready_i)
  `DEF_VALID_READY_COV(read_leaf_resp, clk_i, rst_ni, leaf_resp_valid_i, leaf_resp_ready_o)
  ...
  covergroup cg_read_scoreboard (string name) @(posedge clk_i iff rst_ni);
  ...
  endgroup
  cg_read_scoreboard inst_cg_scoreboard = new($sformatf("%m"));

endmodule

bind tag_lookup_engine_table_lookups_read tag_lookup_engine_table_lookups_read_cov #(
  .sb_entry_t
, .MAX_IN_FLIGHT
) tag_lookup_engine_table_lookups_read_cov_bind (
  .clk_i, .rst_ni
